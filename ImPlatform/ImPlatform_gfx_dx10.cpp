...
static ID3D10SamplerState* g_Samplers[2][3]  = {};
static ID3D10SamplerState* g_SamplerStack[8] = {};
...
IMPLATFORM_API bool ImPlatform_InitGfx(void)
{
    ...
    static const D3D10_FILTER kFilters[2] = { D3D10_FILTER_MIN_MAG_MIP_POINT, D3D10_FILTER_MIN_MAG_MIP_LINEAR };
    static const D3D10_TEXTURE_ADDRESS_MODE kAddr[3] = { D3D10_TEXTURE_ADDRESS_CLAMP, D3D10_TEXTURE_ADDRESS_WRAP, D3D10_TEXTURE_ADDRESS_MIRROR };
    for (int f = 0; f < 2; ++f)
    for (int w = 0; w < 3; ++w)
    {
...
IMPLATFORM_API bool ImPlatform_CopyTexture(ImTextureID dst, ImTextureID src)
{
    ...
    ID3D10ShaderResourceView* pDstSRV = (ID3D10ShaderResourceView*)dst;
    ID3D10ShaderResourceView* pSrcSRV = (ID3D10ShaderResourceView*)src;
    ...
    g_GfxData.pDevice->CopyResource(pDstRes, pSrcRes);
...
IMPLATFORM_API void ImPlatform_PushSampler(ImPlatform_TextureFilter filter, ImPlatform_TextureWrap wrap)
{
    ...
    int w = (int)wrap; if (w < 0 || w > 2) w = 0;
    ID3D10SamplerState* s = g_Samplers[f][w];
    if (!s) return;

    ImGui::GetWindowDrawList()->AddCallback(
        [](const ImDrawList*, const ImDrawCmd* cmd) {
            auto* rs = static_cast<ImGui_ImplDX10_RenderState*>(
                ImGui::GetPlatformIO().Renderer_RenderState);
            if (!rs) return;
            ID3D10SamplerState* newS = static_cast<ID3D10SamplerState*>(cmd->UserCallbackData);
            ID3D10SamplerState* old = nullptr;
            rs->Device->PSGetSamplers(0, 1, &old);
            if (g_SamplerDepth < 8) g_SamplerStack[g_SamplerDepth++] = old;
            else if (old) old->Release();
            rs->Device->PSSetSamplers(0, 1, &newS);
        }, static_cast<void*>(s));
}
...
IMPLATFORM_API void ImPlatform_PopSampler(void)
{
    ImGui::GetWindowDrawList()->AddCallback(
        [](const ImDrawList*, const ImDrawCmd*) {
            ...
            ID3D10SamplerState* saved = g_SamplerStack[--g_SamplerDepth];
            rs->Device->PSSetSamplers(0, 1, &saved);
...
struct ImPlatform_ShaderProgramData_DX10
{
    ...
    ID3D10Buffer* pPixelConstantBuffer;
...
static UINT ImPlatform_DX10_TranslateCompileFlags(unsigned int compile_flags)
{
...
// Try to load cached DXBC bytecode into a fresh ID3DBlob. Returns NULL on miss.
static ID3DBlob* ImPlatform_DX10_TryLoadCachedBytecode(
...
IMPLATFORM_API ImPlatform_Shader ImPlatform_CreateShader(const ImPlatform_ShaderDesc* desc)
{
    ...
    // Resolve target profile. DX10 hardware caps at SM 4.0 (DX10.1 at 4.1);
    // we use vs_4_0/ps_4_0 for maximum compatibility.
    ...
    {
        // Bytecode path: wrap user bytes in an ID3DBlob.
        hr = D3DCreateBlob(desc->bytecode_size, &shader_data->pBlob);
        if (FAILED(hr) || !shader_data->pBlob)
        {
            fprintf(stderr, "DX10 shader bytecode blob allocation failed (size=%u)\n", desc->bytecode_size);
            delete shader_data;
            return NULL;
        }
    ...
    else
    {
        ...
        shader_data->pBlob = ImPlatform_DX10_TryLoadCachedBytecode(
        ...
        if (!shader_data->pBlob)
        {
            ...
            UINT flags = ImPlatform_DX10_TranslateCompileFlags(desc->compile_flags);
            hr = D3DCompile(
...
IMPLATFORM_API void ImPlatform_DestroyShaderProgram(ImPlatform_ShaderProgram program)
{
    ...
    if (program_data->pPixelConstantBuffer) program_data->pPixelConstantBuffer->Release();
    if (program_data->pPixelConstantData) free(program_data->pPixelConstantData);
    delete program_data;
}
